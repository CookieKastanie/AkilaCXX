...
namespace akila {
	...
	public:
		...
		bool getKey(Key key) const { return buttons[static_cast<int>(key)]; }

		int getButtonCount() const override { return GLFW_KEY_LAST + 1; }
	...
	private:
		...
		void setKey(Key key, bool state) { buttons[static_cast<int>(key)] = state; }

		std::array<bool, GLFW_KEY_LAST + 1> buttons;
	};
}
