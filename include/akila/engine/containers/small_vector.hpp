...
namespace akila {
	...
	public:
		...
		explicit SmallVector(size_type count, const T &value = T(), const Allocator &alloc = Allocator()) {
			if(count == N) {
				stack_.fill(value);
			} else if(count < N) {
		...
		SmallVector(const SmallVector &other, const Allocator &alloc = Allocator()):
			stack_(other.stack_),
		...
		SmallVector(SmallVector &&other, const Allocator &alloc = Allocator()):
		...
		SmallVector(std::initializer_list<T> initlist, const Allocator &alloc = Allocator()) {
		...
		SmallVector &operator=(std::initializer_list<value_type> rhs) {
			if(rhs.size() <= N) {
				stack_ = rhs;
			} else {
				heap_ = rhs;
